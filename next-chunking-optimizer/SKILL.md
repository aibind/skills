...
- Use the ask tool (or the host's equivalent user-question tool) whenever asking the user a question, including target selection, missing information, and final approval. Use plain text only if no available tool supports that question.
...
- **Existing Test Environment**: Use the app's existing test environment, data, and login flow. Running this skill does not authorize creating a demo, provisioning a database, generating or seeding sample data, creating test accounts, or adding or bypassing authentication. If the environment, data, or access is missing, ask the user how to access the existing setup and wait before dependent work. Creating a replacement requires an explicit user request.
...
After the version check passes, if the router or configured production bundler is incompatible, report the evidence and the scope change needed. Otherwise attempt the declared production build and serve it using the existing test environment. Verify Turbopack in its output. Routine setup may include starting existing services or choosing an unused port; inspect setup scripts before running them to ensure they respect the Existing Test Environment rule. If build, serving, browser access, or authentication fails, report the concrete blocker and ask for missing environment or access information when needed. Continue independent work while waiting, without constructing a replacement environment. Do not require a pre-existing verified production build before attempting one.
